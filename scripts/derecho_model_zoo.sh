...
CONFIG=${CONFIG:?'CONFIG env var is required, e.g. CONFIG=config/model_zoo/stormer.yml'}

export LSCRATCH=/glade/derecho/scratch/schreck/tmp
...
mpiexec -n ${total_gpus} --ppn ${num_gpus} --cpu-bind none \
    ...
    ${REPO}/applications/train.py -c ${REPO}/${CONFIG}
