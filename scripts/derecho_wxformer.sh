...
# Usage:
#   qsub -v CONFIG=/path/to/model.yml scripts/derecho_wxformer.sh
#
# Single-node test (4 GPUs):
#   qsub -v CONFIG=... -l select=1:ncpus=64:ngpus=4:mem=480GB scripts/derecho_wxformer.sh
#
# 8-node production (32 GPUs):
#   qsub -v CONFIG=... -l select=8:ncpus=64:ngpus=4:mem=480GB scripts/derecho_wxformer.sh
...
CONFIG=${CONFIG:-${REPO}/applications/model.yml}

# ----- NCCL / MPICH tuning (multi-node fabric) --------------------------------
...
mpiexec -n ${total_gpus} --ppn ${num_gpus} --cpu-bind none \
    torchrun \
        --nnodes=${num_nodes} \
