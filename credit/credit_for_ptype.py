...
class CreditPostProcessor:
    ...
    def extract_variable_levels(self, data: xr.Dataset) -> np.ndarray:
        ...
        data_array = np.empty((n_vars, lat_size, lon_size), dtype=np.float32)
    ...
    def grid_predictions(data, predictions,output_uncertainties=False):
        ...
        if output_uncertainties:
            ...
            data['ML_u'] = (['latitude', 'longitude'], u.astype('float32'))
            ...
            for i, (var, v) in enumerate(zip(["Rain", "Snow", "Ice Pellets", "Freezing Rain"],
                ...
                for uncertainty_type, long_name, short_name in zip([ale, epi], ["aleatoric", "epistemic"], ["ale", "epi"]):
                    data[f"ML_{v}_{short_name}"] = (['latitude', 'longitude'], uncertainty_type[:, :, i].astype('float32'))
        ...
        for i, (long_v, v) in enumerate(zip(
            ...
            data[f"ML_{v}"] = (['latitude', 'longitude'], reshaped_preds[:, :, i].astype('float32'))  # ML probability
