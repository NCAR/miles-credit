...
class Metadata:
    ...
    def __post_init__(self):
        ...
        # Validate matrix-valued latitudes and longitudes:
            ...
            raise ValueError("The latitudes and longitudes must either both be vectors or both be matrices.")
...
class Batch:
    ...
    def normalise(self, surf_stats: dict[str, tuple[float, float]]) -> "Batch":
        ...
        """
        return Batch(
            surf_vars={k: normalise_surf_var(v, k, stats=surf_stats) for k, v in self.surf_vars.items()},
            static_vars={k: normalise_surf_var(v, k, stats=surf_stats) for k, v in self.static_vars.items()},
            atmos_vars={k: normalise_atmos_var(v, k, self.metadata.atmos_levels) for k, v in self.atmos_vars.items()},
    ...
    def unnormalise(self, surf_stats: dict[str, tuple[float, float]]) -> "Batch":
        ...
        """
        return Batch(
            surf_vars={k: unnormalise_surf_var(v, k, stats=surf_stats) for k, v in self.surf_vars.items()},
            static_vars={k: unnormalise_surf_var(v, k, stats=surf_stats) for k, v in self.static_vars.items()},
            atmos_vars={k: unnormalise_atmos_var(v, k, self.metadata.atmos_levels) for k, v in self.atmos_vars.items()},
            metadata=self.metadata,
        )
    ...
    def crop(self, patch_size: int) -> "Batch":
        ...
        if h % patch_size == 0:
            ...
            raise ValueError(f"There can at most be one latitude too many, but there are {h % patch_size} too many.")
        ...
        ds = xr.Dataset(
            {
                ...
                **{f"static_{k}": (("latitude", "longitude"), _np(v)) for k, v in self.static_vars.items()},
                **{
