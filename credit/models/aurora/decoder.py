...
class Perceiver3DDecoder(nn.Module):
    ...
    def forward(
        ...
        atmos_levels_encode = levels_expansion(torch.tensor(atmos_levels, device=x.device), self.embed_dim).to(
            dtype=x.dtype
        )
        ...
        head_inputs = [x_atmos if name not in self.separate_perceiver else x_atmos_alternate for name in atmos_vars]
        if not self.level_condition:
            x_atmos = torch.stack(
            ...
            x_atmos = torch.stack(
                [self.atmos_heads[name](x, levels=atmos_levels) for name, x in zip(atmos_vars, head_inputs)],
                dim=-1,
            )
