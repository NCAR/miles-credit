...
from .fourier import lead_time_expansion
from .lora import LoRAMode, LoRARollout
from .util import init_weights, maybe_adjust_windows
        ...
        self.blocks = nn.ModuleList(
            [
                Swin3DTransformerBlock(
                    ...
                    num_heads=num_heads,
                    window_size=ws,
                    shift_size=((0, 0, 0) if (i % 2 == 0) else (ws[0] // 2, ws[1] // 2, ws[2] // 2)),
                    time_dim=time_dim,
                    mlp_ratio=mlp_ratio,
                    qkv_bias=qkv_bias,
                    drop=drop,
...
class Swin3DTransformerBackbone(nn.Module):
    ...
    def __init__(
        ...
        dpr: list[float] = [x.item() for x in torch.linspace(0, drop_path_rate, sum(encoder_depths))]
