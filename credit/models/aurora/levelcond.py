...
class LevelConditioned(nn.Module):
    ...
    def __init__(
        ...
        self.layers = torch.nn.ParameterDict({level_to_str(level): construct_module() for level in levels})

    def forward(self, x: torch.Tensor, *args, levels: tuple[int | float, ...], **kw_args) -> torch.Tensor:
        """Run the module.
        ...
        return torch.stack(
            [self.layers[level_to_str(level)](x[index(i)], *args, **kw_args) for i, level in enumerate(levels)],
            dim=levels_dim,
        )
