...
class GateCell(nn.Module):
    def __init__(self, hidden_size):
        ...
        self.z_x_conv = nn.Conv2d(hidden_size, hidden_size, kernel_size=1)
        self.z_h_conv = nn.Conv2d(hidden_size, hidden_size, kernel_size=1)

        self.r_x_conv = nn.Conv2d(hidden_size, hidden_size, kernel_size=1)
        self.r_h_conv = nn.Conv2d(hidden_size, hidden_size, kernel_size=1)

        self.h_x_conv = nn.Conv2d(hidden_size, hidden_size, kernel_size=1)
        self.h_h_conv = nn.Conv2d(hidden_size, hidden_size, kernel_size=1)

    def forward(self, x, h):
        # x shape: [1, 128, 400, 720]
        z = self.z_x_conv(x)
        r = self.r_x_conv(x)
        ...
        h_hat = self.h_x_conv(x)

        if h is not None:
            h_hat = h_hat + self.h_h_conv(r * h)
...
class Transformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.gated_unit = gated_unit
        if self.gated_unit:
            self.gated_unit = GateCell(dim)
        ...
        **kwargs
        ...
        for (dim_in, dim_out), layers, global_wsz, local_wsz, cel_kernel_sizes, cel_stride in zip(dim_in_and_out, depth, global_window_size, local_window_size, cross_embed_kernel_sizes, cross_embed_strides):
            self.layers.append(nn.ModuleList([
                ...
                Transformer(dim_out, local_window_size=local_wsz, global_window_size=global_wsz, depth=layers, dim_head=dim_head, attn_dropout=attn_dropout, ff_dropout=ff_dropout, gated_unit=gated_cell)
            ]))
