...
def fast_state_interp_loop(
    ...
    for (i, j), p in np.ndenumerate(surface_pressure_data):
        ...
        interp_full_data["P"], full_half_pressure_grid = create_pressure_grid(surface_pressure_data[i, j], a_half_full, b_half_full)
        ...
        for interp_field in interp_fields:
            if interp_field == temperature_var:
                pressure_ds[interp_field + pres_ending][:, i : i + 1, j : j + 1] = interp_temperature_to_pressure_levels(
