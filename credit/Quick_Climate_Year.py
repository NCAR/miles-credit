...
def run_year_rmse(config, input_shape, forcing_shape, output_shape, device, model_name=None):
    ...
    DS_forcx_dynamic = DSforc_norm[df_variables].isel(time=slice(0, num_ts)).load()
...
def run_year_rmse_online(config, input_shape, forcing_shape, output_shape, device):
    with open(config) as cf:
        conf = yaml.load(cf, Loader=yaml.FullLoader)
