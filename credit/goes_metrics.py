...
class LatWeightedMetrics:
    def __init__(self, conf, training_mode=True):
        self.conf = conf
        channels = conf["data"]["channels"]
        self.vars = [f"C{channel:02}" for channel in channels]

        self.w_lat = None
        if conf["loss"]["use_latitude_weights"]:
            self.w_lat = latitude_weights(conf)
...
if __name__ == "__main__":
    ...
    with open("config/goes_example.yml") as cf:
    ...
    true = torch.tensor(np.random.rand(1, 6, 1003, 923), dtype=torch.float32)
    pred = torch.tensor(np.random.rand(1, 6, 1003, 923), dtype=torch.float32)
