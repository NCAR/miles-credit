...
class Trainer(BaseTrainer):
    ...
    def _compute_distributed_ensemble_loss(self, y, y_pred, criterion, ensemble_size, is_training=True):
        ...
        # --- 2. GATHER THE ENTIRE TENSOR ONCE (No Autograd Overhead) ---
        ...
        for i in range(0, lat_size, chunk_size):
            ...
            unweighted_slice_loss = criterion(y_slice.to(y_pred_slice.dtype), y_pred_slice).mean()
            
            if lat_weights is not None:
                chunk_weight = lat_weights[i:end_i].mean() * ((end_i - i) / lat_size)
                loss = unweighted_slice_loss * chunk_weight
            else:
                loss = unweighted_slice_loss * ((end_i - i) / lat_size)
