...
class SKEBS(nn.Module):
    ...
    def __init__(self, post_conf):
        ...
        if self.dissipation_type == "prescribed":
            self.backscatter_network = BackscatterPrescribed(self.nlat, self.nlon, self.levels, post_conf["data"]["std_path"], post_conf["skebs"]["sigma_max"])
    ...
    def initialize_skebs_parameters(self):
        ...
        self.register_buffer(
            ...
            torch.cat([torch.zeros(self.zero_out_levels_top_of_model), torch.ones(self.levels - self.zero_out_levels_top_of_model)]).view(1, self.levels, 1, 1, 1),
            persistent=False,
        )
        if self.tropics_only_dissipation:
            self.register_buffer("tropics_backscatter_filter", torch.cat([torch.zeros(70), torch.ones(52), torch.zeros(70)]).view(1, 1, 1, self.nlat, 1), persistent=False)
        else:
            self.register_buffer("tropics_backscatter_filter", torch.ones(self.nlat).view(1, 1, 1, self.nlat, 1), persistent=False)
    ...
    def initialize_filters(self):
        ...
        def filter_init(max_wavenum, anneal_start):
            filter = torch.cat([torch.ones(anneal_start), torch.linspace(1.0, 0.2, max_wavenum - anneal_start), torch.zeros(self.lmax - max_wavenum)])
