    ...
    for argname in list(argdict):
        if argname not in ignore:
            if argname not in sig.parameters:
                del argdict[argname]
                warnings.warn(f"'{argname}' is not a valid parameter for {function};" + f" removing from configuration for {context}")
    return argdict
...
def credit_main_parser(conf, parse_training=True, parse_predict=True, print_summary=False):
    ...
    if is_ocean:
        ...
        for dset in dconf:
            ...
            for usage in dconf[dset]["variables"]:
                if usage not in vartypes:
                    raise ValueError(f"unknown variable usage '{usage}' for dataset {dset};" + f" must be one of {vartypes}")
        ...
        conf["data"]["all_varnames"] = (
            conf["data"]["variables"] + conf["data"]["surface_variables"] + conf["data"]["dynamic_forcing_variables"] + conf["data"]["diagnostic_variables"]
        )
...
def training_data_check(conf, print_summary=False):
    ...
    assert all(varname in varnames_upper_air for varname in conf["data"]["variables"]), "upper-air variables [{}] are not fully covered by conf['data']['save_loc']".format(
        conf["data"]["variables"]
    )
    ...
    if conf["data"]["flag_surface"]:
        ...
        assert all(varname in varnames_surface for varname in conf["data"]["surface_variables"]), (
            "Surface variables [{}] are not fully covered by conf['data']['save_loc_surface']".format(conf["data"]["surface_variables"])
        )
    ...
    if conf["data"]["flag_dyn_forcing"]:
        ...
        assert all(varname in varnames_dyn_forcing for varname in conf["data"]["dynamic_forcing_variables"]), (
            "Dynamic forcing variables [{}] are not fully covered by conf['data']['save_loc_dynamic_forcing']".format(conf["data"]["dynamic_forcing_variables"])
        )
    ...
    if conf["data"]["flag_diagnostic"]:
        ...
        assert all(varname in varnames_diagnostic for varname in conf["data"]["diagnostic_variables"]), (
            "Diagnostic variables [{}] are not fully covered by conf['data']['save_loc_diagnostic']".format(conf["data"]["diagnostic_variables"])
        )
    ...
    if conf["data"]["flag_forcing"]:
        ...
        assert all(varname in varnames_forcing for varname in conf["data"]["forcing_variables"]), (
            "Forcing variables [{}] are not fully covered by conf['data']['save_loc_forcing']".format(conf["data"]["forcing_variables"])
        )
    ...
    if conf["data"]["flag_static"]:
        ...
        assert all(varname in varnames_static for varname in conf["data"]["static_variables"]), (
            "Static variables [{}] are not fully covered by conf['data']['save_loc_static']".format(conf["data"]["static_variables"])
        )
    ...
    if conf["data"]["flag_dyn_forcing"]:
        ...
        for coord_name in coord_dyn_forcing:
            assert ds_upper_air.coords[coord_name].equals(ds_dyn_forcing.coords[coord_name]), "coordinate {} mismatched between upper-air and dynamic forcing files".format(
                coord_name
            )
