...
class OutputWrangler:
    ...
    def __post_init__(self):
        ...
        for dset in self.templates['files']:
            dmap = dataset.datasets[dset]['datamap']

            path = os.path.join(self.templates['dir'], self.templates['files'][dset])
    ...
    def process(self, y_pred, dates):
        ...
        hlen = dataset.history_len
        
        datadict = dataset.revert(y_pred)
...
class OutputWriter:
    ...
    def __post_init__(self):
        ...
        for usage in ("prognostic", "diagnostic"):
            if usage in variables:
                self.outvars = self.outvars + variables[usage]
        ...
        if len(self.outvars) == 0:
            ...
            self.template = xr.open_dataset(self.path, decode_times=False)
            self.template.load()
            self.template.close()
            ...
            for v in self.template.data_vars:
                if v in self.outvars:
                    ...
                    self.template[v][:] = numpy.nan                    
                else:
                    self.template = self.template.drop(v)

    def write(self, data, newtime, path):
