...
class GOES10kmDataset(Dataset):
    def __init__(
        ...
        if not self.evaluate:
            self.init_times = (
                self._timestamps()
            )  # will generate valid init times if needed
            logger.info(
                f"initializing GOES10kmDataset with timestep [{self.timestep}] with {self.num_forecast_steps} steps"
            )
        else:
        ...
        if "rollout_init_times" in time_config.keys():
            logger.info(
                "setting up GOES 10km dataset for rollout mode by subsetting times"
            )
            self._rollout_mode(
    ...
    def _rollout_mode(self, rollout_init_times, time_tol):
        ...
        self.init_times = self.init_times.sel(
            t=rollout_init_times,
            method="nearest",
    ...
    def _generate_valid_init_times(self, valid_init_filepath):
        ...
        def check_valid_forecast_times(t_init, timestep, num_forecast_steps):
            ...
            target_times = [
                t_init.values + timestep * step
                for step in range(1, num_forecast_steps + 1)
            ]
            zarr_times = self.ds.t.sel(t=target_times, method="nearest")
            within_tol = (
                zarr_times.values - np.array(target_times).astype(zarr_times.dtype)
            ) < time_tolerance

            return all(within_tol)

        logger.info(
            f"generating valid init times and saving to {valid_init_filepath}. will take around 5 min"
        )
