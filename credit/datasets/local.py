...
class LocalDataset(BaseDataset):
        ...
        *,
        ...
        if t_history is None:
            with xr.open_dataset(_find_file(file_intervals, t)) as ds:
                ...
                self._write_field_tensors(
                    ds_t, vars_3D, vars_2D, field_type, sample, time_size=1
                )
            return
        ...
        n_t = len(t_history)
        per_var_3D: dict[str, list[torch.Tensor]] = {v: [None] * n_t for v in vars_3D}
        per_var_2D: dict[str, list[torch.Tensor]] = {v: [None] * n_t for v in vars_2D}
        ...
        for path, indices in groups.items():
            with xr.open_dataset(path) as ds:
                ...
                if not has_time:
                    ...
                    ds_t = ds
                    arrs_3D = {
                        v: self._read_3d_array(ds_t, v) for v in vars_3D
                    }
                    arrs_2D = {v: ds_t[v].values for v in vars_2D}
                    for k in indices:
                        for v in vars_3D:
                            per_var_3D[v][k] = torch.tensor(
                                arrs_3D[v], dtype=torch.float32
                            ).unsqueeze(1)  # (n_lev, 1, lat, lon)
                        for v in vars_2D:
                            per_var_2D[v][k] = (
                                torch.tensor(arrs_2D[v], dtype=torch.float32)
                                .unsqueeze(0)
                                .unsqueeze(0)
                            )  # (1, 1, lat, lon)
                else:
        ...
        for v in vars_3D:
            tensor = torch.cat(per_var_3D[v], dim=1)
            key = self._get_field_name(field_type, "3d", v)
            sample[key] = tensor
        for v in vars_2D:
            tensor = torch.cat(per_var_2D[v], dim=1)
    ...
    def _write_field_tensors(
        ...
        Used by the legacy single-step path.
        """
