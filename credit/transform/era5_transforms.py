...
class ERA5StandardTransform:
    def __init__(self, conf, device):
        ...
        self.mean_ds = xr.open_dataset(
            source_conf["prognostic"]["transform"]["mean_path"]
        ).sortby("level", ascending=True)
        self.std_ds = xr.open_dataset(
            source_conf["prognostic"]["transform"]["std_path"]
        ).sortby("level", ascending=True)

        # setup tensor scaling, use variables list from config to enforce order
        mean_3D_tensor = torch.tensor(
            self.mean_ds[vars_3D]
            .to_array()
            .stack({"level_var": ["variable", "level"]})
            .values
        )
        mean_2D_tensor = torch.tensor(self.mean_ds[vars_2D].to_array().values)
        self.mean_tensor = torch.cat([mean_3D_tensor, mean_2D_tensor], axis=0).to(
            device
        )

        std_3D_tensor = torch.tensor(
            self.std_ds[vars_3D]
            .to_array()
            .stack({"level_var": ["variable", "level"]})
            .values
        ...
        self.std_tensor = std_tensor.view(1, self.num_prognostic_variables, 1, 1, 1).to(
            device
        )
    ...
    def transform_tensor(self, tensor):
        ...
        prognostic_transformed = (
            prognostic_tensor - self.mean_tensor
        ) / self.std_tensor

        return torch.cat(
            [prognostic_transformed, tensor[-self.num_prognostic_variables :]]
        )
    ...
    def inverse_transform_tensor(self, tensor):
        ...
        return torch.cat(
            [prog_inv_transformed, tensor[-self.num_prognostic_variables :]]
        )
