    ...
    def setup_test_files(self):
        ...
        for file_idx in range(self.num_files):
            ...
            with nc.Dataset(file_path, "w") as ds:
                ...
                start_minutes = (
                    global_start_timestep * 1440
                )  # 1440 min/day constant delta
                ...
                prognostic_var = ds.createVariable(
                    "prognostic_field", "f4", ("time", "x", "y")
                )
                for t in range(self.timesteps_per_file):
                    base_value = (
                        (file_idx * 1000) + t + 100
                    )  # Offset to distinguish from boundary
                    spatial_pattern = np.arange(
                        self.spatial_dims[0] * self.spatial_dims[1]
                    ).reshape(self.spatial_dims)
                ...
                diagnostic_var = ds.createVariable(
                    "diagnostic_field", "f4", ("time", "x", "y")
                )
                for t in range(self.timesteps_per_file):
