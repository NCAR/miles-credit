...
def create_synthetic_data() -> dict:
    ...
    shape = (100, 16, 1, 8, 8)
    var_names = [
        "Test_ERA5/era5/pronostic/3d/T",
        "Test_ERA5/era5/pronostic/3d/U",
        "Test_ERA5/era5/pronostic/3d/V",
    ]

    return {"Test_ERA5": {split: {var: torch.randn(*shape) for var in var_names} for split in ("input", "target")}}
...
def test_regrid_output_shape(weight_file):
    ...
    variables = ["Test_ERA5/era5/pronostic/3d/T"]
    regrid = Regridder(path, variables=variables)
    batch = create_synthetic_data()
    result = regrid(batch)
    for split in ("input", "target"):
        assert result["Test_ERA5"][split]["Test_ERA5/era5/pronostic/3d/T"].shape == (100, 16, 1, n_dst_lat, n_dst_lon)
...
def test_regrid_uniform_input(weight_file):
    ...
    variables = ["Test_ERA5/era5/pronostic/3d/T"]
    regrid = Regridder(path, variables=variables)
    batch = {"Test_ERA5": {"input": {"Test_ERA5/era5/pronostic/3d/T": torch.ones(1, 1, 1, n_src_lat, n_src_lon)}}}
    result = regrid(batch)
    assert torch.allclose(
        result["Test_ERA5"]["input"]["Test_ERA5/era5/pronostic/3d/T"],
...
def test_regrid_reshape_false(weight_file):
    ...
    batch = create_synthetic_data()
    result = regrid(batch)
    assert result["Test_ERA5"]["input"]["Test_ERA5/era5/pronostic/3d/T"].shape == (100 * 16 * 1, n_dst_lat * n_dst_lon)
...
def test_regrid_flip_axis(weight_file):
    ...
    assert not torch.allclose(
        result["Test_ERA5"]["input"]["Test_ERA5/era5/pronostic/3d/T"],
        result_flip["Test_ERA5"]["input"]["Test_ERA5/era5/pronostic/3d/T"],
    )
...
def scaler_file(tmp_path):
    ...
    variables = x_dict["Test_ERA5"]["input"].keys()
...
def test_scaler_output_shape(scaler_file):
    ...
    original_shapes = {v: data["Test_ERA5"]["input"][v].shape for v in variables}
    result = scaler(data)
    for v in variables:
        assert result["Test_ERA5"]["input"][v].shape == original_shapes[v]
...
def test_scaler_transform_changes_values(scaler_file):
    ...
    original = data["Test_ERA5"]["input"][var].clone()
    result = scaler(data)
    assert not torch.allclose(result["Test_ERA5"]["input"][var].float(), original.float())
...
def test_scaler_round_trip(scaler_file):
    ...
    data = fwd(data)
    data = inv(data)
    assert torch.allclose(data["Test_ERA5"]["input"][var].float(), original.float(), atol=1e-5)
