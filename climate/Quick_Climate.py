    ...
    for block_start in range(start_ix, start_ix + num_ts, chunk_size):
        ...
        for t in range(gpu_forcing_chunk.shape[0]):
            ...
            if isinstance(time_obj, datetime):
                ...
                utc_datetime = datetime(time_obj.year, time_obj.month, time_obj.day, time_obj.hour, time_obj.minute, time_obj.second)
            ...
            upper_air, single_level = make_xarray(prediction.cpu(), utc_datetime, latlons.latitude.values, latlons.longitude.values, conf)

            # Async save to NetCDF (runs in background pool)
            pool.apply_async(save_netcdf_increment, (upper_air, single_level, init_str, lead_time_periods * forecast_hour, metadata, conf))
...
Example usage:
    ...
    parser.add_argument("--model_name", type=str, default=None, help="Optional model checkpoint name (e.g., checkpoint.pt00091.pt)")
    parser.add_argument("--save_append", type=str, default=None, help="Append subfolder name to output directory")
    parser.add_argument("--device", type=str, default="cuda", help="Device to run on (cuda or cpu)")
    parser.add_argument("--init_noise", type=float, default=None, help="Add Gaussian noise to initial conditions (for ensembles)")

    # Deprecated arguments (kept for backwards compatibility but unused)
    parser.add_argument("--input_shape", type=int, nargs="+", default=None, help="[DEPRECATED] Input shape is now derived from config")
    parser.add_argument("--forcing_shape", type=int, nargs="+", default=None, help="[DEPRECATED] Forcing shape is now derived from config")
    parser.add_argument("--output_shape", type=int, nargs="+", default=None, help="[DEPRECATED] Output shape is now derived from config")
    ...
    with mp.Pool(num_cpus) as pool:
        flag_energy = run_climate_integration(pool=pool, context=context, save_append=args.save_append, init_noise=args.init_noise)
