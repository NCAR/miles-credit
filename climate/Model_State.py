...
class StateVariableAccessor:
    ...
    def _build_state_indices(self):
        ...
        for var in self.prognostic_vars:
            indices[var] = {"start_idx": idx, "end_idx": idx + self.levels, "n_channels": self.levels, "is_3d": True, "available": True}
            idx += self.levels
    ...
    def get_var_info(self, var_name: str) -> Dict:
        ...
        if var_name not in indices:
            all_vars = self.prognostic_vars + self.surface_vars + self.diagnostic_vars + self.dynamic_forcing_vars + self.forcing_vars + self.static_vars
    ...
    def get_state_var(self, state_tensor: torch.Tensor, var_name: str, time_idx: Optional[int] = None) -> torch.Tensor:
        ...
        if not info["available"]:
            raise ValueError(f"Variable '{var_name}' not available in '{self.tensor_type}' tensor. Reason: {info.get('reason', 'Unknown')}")
        ...
        if time_idx is not None:
            if time_idx >= state_tensor.shape[2]:
                raise IndexError(f"Time index {time_idx} out of bounds for tensor with {state_tensor.shape[2]} time steps")
            var_slice = var_slice[:, :, time_idx, :, :]

        return var_slice

    def set_state_var(self, state_tensor: torch.Tensor, var_name: str, var_data: torch.Tensor, time_idx: Optional[int] = None) -> None:
        """
...
class StateManager:
    ...
    def build_input_with_forcing(self, state: torch.Tensor, dynamic_forcing: torch.Tensor, static_forcing: torch.Tensor) -> torch.Tensor:
        """
...
class CAMulatorStepper:
    ...
    def _setup_postprocessing(self):
        ...
        self.flag_energy = POSTBLOCK_AVAILABLE and post_conf["activate"] and post_conf["global_energy_fixer"]["activate"]
    ...
    def get_state_var(self, tensor: torch.Tensor, var_name: str, tensor_type: Literal["state", "input", "output"] = "state", time_idx: Optional[int] = None) -> torch.Tensor:
        """
        ...
        Returns:
            Variable tensor
        """
        accessor = {"state": self.state_accessor, "input": self.input_accessor, "output": self.output_accessor}[tensor_type]

        return accessor.get_state_var(tensor, var_name, time_idx)

    def set_state_var(
        self, tensor: torch.Tensor, var_name: str, var_data: torch.Tensor, tensor_type: Literal["state", "input", "output"] = "state", time_idx: Optional[int] = None
    ) -> None:
        """
...
def initialize_camulator(config_path: str, model_name: str = None, device: str = "cuda") -> dict:
    ...
    if not CREDIT_AVAILABLE:
        raise ImportError("CREDIT modules not available. Cannot initialize CAMulator. Please ensure credit package is installed and importable.")
    ...
    if not os.path.exists(ic_path):
        raise FileNotFoundError(f"Initial condition file not found: {ic_path}\nPlease run Make_Climate_Initial_Conditions.py first.")
