...
def save_netcdf_increment(
    ...
    try:
        ...
        if "climate_rescale_output" in conf["predict"].keys():
            #state_transformer = load_transforms(conf) if conf["data"]["scaler_type"] == "std_new" else _not_supported()
            #ds_merged = state_transformer.transforms[0].inverse_transform_dataset(ds_merged)
