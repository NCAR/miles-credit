...
# example:
# python Post_Process.py /glade/derecho/scratch/wchapman/CREDIT/cesm_wxformer_00168/2001-01-01T00Z/ 1D PRECT TREFHT
# python Post_Process.py /glade/derecho/scratch/wchapman/CREDIT/cesm_wxformer_00184/2001-01-01T00Z/ 1D PRECT TREFHT
# python Post_Process.py /glade/derecho/scratch/wchapman/CREDIT/cesm_wxformer_00185/2001-01-01T00Z/ 1D PRECT TREFHT
# python Post_Process.py /glade/derecho/scratch/wchapman/CREDIT/cesm_wxformer_00186/2001-01-01T00Z/ 1D PRECT TREFHT
# python Post_Process.py /glade/derecho/scratch/wchapman/CREDIT/cesm_wxformer_00187/2001-01-01T00Z/ 1D PRECT TREFHT
# python Post_Process.py /glade/derecho/scratch/wchapman/CREDIT/cesm_wxformer_00188/2001-01-01T00Z/ 1D PRECT TREFHT
# python Post_Process.py /glade/derecho/scratch/wchapman/CREDIT/cesm_wxformer_00189/2001-01-01T00Z/ 1D PRECT TREFHT
# python Post_Process.py /glade/derecho/scratch/wchapman/CREDIT/cesm_wxformer_00190/2001-01-01T00Z/ 1D PRECT TREFHT
# python Post_Process.py /glade/derecho/scratch/wchapman/CREDIT/cesm_wxformer_00191/2001-01-01T00Z/ 1D PRECT TREFHT

# python Post_Process.py config.yml 1D --variables TREFHT PRECT --reset_times --dask_do
# python Post_Process.py config.yml None
...
def post_process(conf, avg_window, variables, name_string='',reset_times=True, dask_do=False, monthly=False, rescale_it=False):
    """
    ...
    if rescale_it == True:
        for fn  in FNS:
            ...
            # Check if the dataset has already been rescaled
            if DSfirst.attrs.get('scaling') == 'rescaled':
                ...
                for var in DSfirst.data_vars:
                    if var in mean_ and var in std_:
                        ...
                        # Broadcast mean and std to (1, 32, 1, 1) and transpose to match dimensions
                        if 'level' in DSfirst[var].dims:
                            mean_broadcast = mean_[var].expand_dims({'time': 1, 'latitude': 192, 'longitude': 288}).transpose('time', 'level', 'latitude', 'longitude')
                            
                            std_broadcast = std_[var].expand_dims({'time': 1, 'latitude': 192, 'longitude': 288}).transpose('time', 'level', 'latitude', 'longitude')
            
                            # Rescale using broadcasting
                            DSfirst[var][:] = ((DSfirst[var].values * std_broadcast.values) + mean_broadcast.values)
                        else:
                            # Rescale for non-level variables directly
                ...
                # Save to a temporary file and replace the original
                temp_fn = fn + ".tmp.nc"
                DSfirst.to_netcdf(temp_fn)
                DSfirst.close()
                os.sync()  # Ensure data is written to disk
                os.replace(temp_fn, fn)  # Atomically replace the file
    ...
    if collect_times:
        if os.path.exists(f'{source_dir}/datetimes_in_files.nc'):
            ...
            forcing_file_path = conf_pred['forcing_file']
            num_files = len(sorted(glob.glob(f'{source_dir}/pred*.nc')))
            DSforc = xr.open_dataset(forcing_file_path)
            init_time = init_date_str
            init_index_ = DSforc['time'].to_index().get_loc(init_time).start
            init_end_ = init_index_ + num_files
            print(f"Index of {init_time}: {init_index_}")
    
            # Create DataArrays
            time_array = DSforc.isel(time=slice(init_index_,init_end_))['time'][1:]
            index_array = xr.DataArray(np.arange(len(time_array)), dims='time', name='index')
