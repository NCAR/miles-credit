        ...
        for var in DSfirst.data_vars:
            if var in mean_ and var in std_:
                ...
                if "level" in DSfirst[var].dims:
                    ...
                    DSfirst[var][:] = (
                        DSfirst[var].values * std_broadcast.values
    ...
    if rescale_it:
        ...
        if not FNS:
            raise FileNotFoundError(
                "No prediction files found in the source directory."
            )
    ...
    if avg_window:
        ...
        for yml in year_month_list:
            avg_file = os.path.join(
                avg_dir, f"averaged_{name_string}_{yml}_{avg_window}.nc"
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Post-process NetCDF files and perform averaging."
    )
    ...
    parser.add_argument(
        "avg_window",
        type=str,
        help="Averaging window (e.g., '1D'). Use 'None' to skip.",
    )
    parser.add_argument(
        "--name_string", type=str, default="", help="Name identifier for output files."
    )
    parser.add_argument(
        "--variables",
        nargs="+",
        help="Variables to process (e.g., TREFHT PRECT). Default is all.",
    )
    parser.add_argument(
        "--reset_times",
        type=str2bool,
    ...
    parser.add_argument(
        "--n_processes", type=int, default=1, help="number of parallel processes"
    )
