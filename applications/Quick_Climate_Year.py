...
def run_year_rmse(config, input_shape, forcing_shape, output_shape, device, model_name=None):
    ...
    DS_forcx_dynamic = DSforc_norm[df_variables].isel(time=slice(indx_start, indx_start+num_ts)).load()
    ...
    plt.tight_layout()
    plt.savefig(f'{conf["save_loc"]}/{fsout}_quick_climate_plot_years.png', bbox_inches='tight')
    plt.show()
