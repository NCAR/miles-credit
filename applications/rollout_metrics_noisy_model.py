...
def compute_spread_skill_metric(all_files, ensemble_size):
    ...
    for df in all_files:
        ...
        std_vars = [col.replace("_std", "").replace("rmse_", "") for col in df.columns if "rmse_" in col and "_std" in col]
    ...
    with torch.no_grad():
        ...
        for k, batch in enumerate(data_loader):
            ...
            if ensemble_size > 1:
                ...
                ensemble_predictions_zscore = y_pred_zscore.permute(1, 0, *range(2, y_pred_zscore.ndim))  # [ensemble_size, batch_size, ...]
