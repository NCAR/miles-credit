    ...
    def find_start_stop_indices(self, index):
        ...
        for init_time in self.init_time_list_np:
            for i_file, ds in enumerate(self.all_files):
                ...
                if isinstance(ds["time"][0].values.item(), cftime.DatetimeNoLeap):
...
def predict(rank, world_size, conf, p):
    ...
    DSforc = DSforc[df_sf_variables].load()
    DSforc_norm = DSforc_norm.load()
    print('forcing file opened and loaded')
    print('################')
    print(DSforc)
    print('################')
    print('################')
    print(df_sf_variables)
    print('################')
    print('################')
    print(DSforc_norm)
    print('################')
    ...
    with torch.no_grad():
        ...
        for k, batch in enumerate(data_loader):
            ...
            print('this is the x shape: ', x.shape)
            # torch.save(x, './init_wxformer_1979_01_01_00Z_tensor.pth')
            # break
            # # To reload it later
            # x_reloaded = torch.load('./x_tensor.pth')
            # -------------------------------------------------------------------------------------- #
            # start prediction
            ...
            y_pred = model(x)
            # print('this is the y shape: ', y_pred.shape)
            # torch.save(y_pred, './pred_01_wxformer_1981_01_01_00Z_tensor.pth')
            # break
            ...
            break
        add_it = 0
        print('initial climate loop forecast count: ', forecast_count)
        for k in range(conf["predict"]["climate_timesteps"]):
            ...
            #add if statement jump here for leap year option... 
            ...
            if k ==0 and "x_forcing_static" in batch:
                ...
                print('the forecast time in climate loop is:', selected_data['time'])
                time_selection = utc_datetime.strftime('%Y-%m-%d %H:%M:%S')
                print('the utc datetime in climate loop is:', utc_datetime.strftime('%Y-%m-%d %H:%M:%S'))
                print('the utc datetime in climate loop is:', utc_datetime)
            ...
            if conf["data"]["static_first"]:
                ...
                # Extract the scalar cftime object
                cftime_obj = DSforc_norm[dfv].isel(time=forc_time_index+add_it)['time'].values
                # Convert to string directly using str()
                cftime_str = str(cftime_obj)  # This will look like '1981-01-02 12:00:00'
