...
def run_year_rmse(p, config, input_shape, forcing_shape, output_shape, device, model_name=None, init_noise=None):
    ...
    outpath = conf_pred['save_forecast']
    ...
    MA_tensor = torch.zeros(output_shape).to(device)
    ...
    for k in range(num_ts):
        ...
        curr_month = (cftime_obj + timedelta(hours=6)).month
        new_cftime_obj = cftime_obj + timedelta(hours=6)
        utc_datetime = datetime.strptime(cftime_str, '%Y-%m-%d %H:%M:%S')
        
        # print('Forcing time:', time.time() - start_forcing)
        if (cftime_obj.day == 1) & (cftime_obj.month == 1) & (cftime_obj.hour == 0):            
            torch.save(x, f'{outpath}/wxformer_init_cond_{cftime_str[:10]}_tensor.pth')
        ...
        # print('cloned')
        MA_tensor.add_(state_transformer.inverse_transform(y_pred_clone))
        # print('added')

        if month_do != curr_month:
            newtime = f'{cftime_obj.year}-{month_do}-01 00:00:00'
            datesend = datetime.strptime(newtime, '%Y-%m-%d %H:%M:%S')
            
            print('Saving Month: ', newtime)
            darray_upper_air, darray_single_level = make_xarray(
                MA_tensor.cpu()/(num_up+1),
                datesend,
            ...
            num_up = 0
            month_do = curr_month
            MA_tensor = MA_tensor*0
        else:
            #handle feb 29th: 
            dd = (cftime_obj + timedelta(hours=6))
            if (dd.month == 2) & (dd.day == 29):
                newtime = f'{cftime_obj.year}-{month_do}-01 00:00:00'
                datesend = datetime.strptime(newtime, '%Y-%m-%d %H:%M:%S')
