...
def main():
    ...
    credit_grid = xr.open_dataset(config["level_meta"])
    model_levels = pd.read_csv(config["model_levels"])
    model_level_indices = model_levels["model_level_indices"].values
    variables = config["upper_air_variables"] + config["surface_variables"]
    date = pd.Timestamp(config["init_time"], tz="UTC")
