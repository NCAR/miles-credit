...
class ForecastProcessor:
    ...
    def process(self, y_pred, forecast_step, forecast_count, datetimes, save_datetimes):
        try:
            ...
            utc_datetimes = [datetime.utcfromtimestamp(datetimes[i].item() / 1e9) + timedelta(hours=self.lead_time_periods) for i in range(self.batch_size)]
...
def predict(rank, world_size, conf, p):
    ...
    static_dim_size = len(conf["data"].get("dynamic_forcing_variables", [])) + len(conf["data"].get("forcing_variables", [])) + len(conf["data"].get("static_variables", []))
    ...
    with torch.no_grad():
        ...
        for batch in data_loader:
            ...
            if forecast_step == 1:
                ...
                init_datetimes = [datetime.utcfromtimestamp(batch["input_datetime"][i].item() / 1e9).strftime("%Y-%m-%dT%HZ") for i in range(batch_size)]
