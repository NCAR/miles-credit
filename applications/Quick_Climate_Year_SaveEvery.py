...
def run_year_rmse(conf, config, p, model, input_shape, forcing_shape, output_shape, device, model_name=None, rank=None, world_size=None):
    ...
    DS_forcx_dynamic = DSforc_norm[df_variables].isel(time=slice(indx_start, indx_start+num_ts+1)).load()
    ...
    plt.tight_layout()
    plt.savefig(f'{conf["save_loc"]}/{fsout}_quick_climate_plot.png', bbox_inches='tight')
    plt.show()
...
def main():
    ...
    if args.model_name is not None:
        print('loading custom: ', args.model_name)
        model = load_model_name(conf, args.model_name, load_weights=True).to(device)
    else:
        model = load_model(conf, load_weights=True).to(device)
    
    print('jit trace model start')
    model = torch.jit.trace(model, x)  # JIT trace in the main process
    print('jit trace model done')
    ...
    with mp.Pool(num_cpus) as p:
        if conf["predict"]["mode"] in ["fsdp", "ddp"]:  # multi-gpu inference
            print('doing ddp')
            _ = run_year_rmse(conf, config, p, model,
            ...
            _ = run_year_rmse(conf, config, p, model,
                ...
                output_shape=args.output_shape,
                model_name=args.model_name,
                device=args.device, rank = 0, world_size = 1)
