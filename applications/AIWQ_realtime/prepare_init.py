...
ds_dynamic_forcings_forecast.to_zarr("/glade/derecho/scratch/cbecker/CREDIT_runs/AIWQ_inits/"+savefi, mode='w', consolidated=True)
...
for GEFS_fi in all_mems:
    print(GEFS_fi)
    ds_GEFS_ICS = xr.open_dataset(ddir_GEFS+GEFS_fi).load()
    os.remove(ddir_GEFS+GEFS_fi)
    ds_GEFS_ICS['level'] = ds_dynamic_forcings.level.values
    ds_GEFS_ICS.to_netcdf(ddir_GEFS+GEFS_fi)
    ds_GEFS_ICS.close()
