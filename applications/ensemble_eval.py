...
from credit.verification.ensemble import (
    binned_spread_skill,
    rank_histogram_apply,
    spread_error,
)
from credit.verification.standard import (
    average_div_rot_spectrum,
...
def do_eval(forecast_save_loc, conf, model_conf, fh):
    ...
    for variable in conf["variables"]:
        for level in conf["levels"]:
            ...
            result_dict = result_dict | _do_standard_eval_on_variable(
                w_lat, da_pred, da_true, variable, level
            )
    ...
    if "U" in conf["variables"] and "V" in conf["variables"]:
        ...
        for level in conf["levels"]:
            ...
            vrt_pred, div_pred = average_div_rot_spectrum(
                ds_pred, conf["grid"], wave_spec="n"
            )
            vrt_true, div_true = average_div_rot_spectrum(
                ds_true, conf["grid"], wave_spec="n"
            )

            result_dict = result_dict | {
                f"vrt_spectrum_{level}": vrt_pred,
                f"div_spectrum_{level}": div_pred,
            }
            result_dict = result_dict | {
    ...
    for variable in conf["single_level_variables"]:
        ...
        # compute and merge dicts
        result_dict = result_dict | _do_standard_eval_on_variable(
            w_lat, da_pred, da_true, variable, None
        )
        result_dict = result_dict | _do_special_eval_on_variable(
            w_lat, conf, fh, da_pred, da_true, variable, None
        )
...
def check_rollout_files(forecast_save_loc):
    ...
    rollout_dirs = [
        dir
        for dir in os.listdir(forecast_save_loc)
        if os.path.isdir(join(forecast_save_loc, dir))
    ]
    num_files = []
    ...
    assert all(
        num == num_files[0] for num in num_files
    ), "not all rollouts have the same number of files"
    return num_files[0]
