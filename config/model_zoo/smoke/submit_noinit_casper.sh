#!/bin/bash
# Submit noinit (random init, same architecture) smoke tests for all 5 pretrained models.
# Compare results against smoke_*_pretrained_casper.yml to see what pretrained weights give.
set -euo pipefail
...
CONFIGS=(
    smoke_stormer_noinit_casper.yml
    smoke_climax_noinit_casper.yml
    smoke_fourcastnet_noinit_casper.yml
    smoke_aurora_noinit_casper.yml
    smoke_fuxi_noinit_casper.yml
)
